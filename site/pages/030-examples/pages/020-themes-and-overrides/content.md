...
viewport height. Every carousel is height-limited automatically with either
method. Both methods use the page's content area after any persistent navigation
has been excluded; they are never positioned against the complete browser
window. Use a page override only when that page's role differs from the preset;
individual image blocks do not choose their own method.
