# Media and surfaces

This example is a visual test bench for Norna's presentation features.

[Live demo](https://janga.github.io/norna/examples/feature-demos/media-and-surfaces/)
|
[View source](https://github.com/janga/norna/tree/main/examples/feature-demos/media-and-surfaces)

It shows managed image stacks, portrait and landscape carousels, card lists and
sidenotes alongside site-wide Light and Dark appearances, the palette, and section
surfaces. The SVG files are hand-authored local assets so the example does not
depend on external images.

Run it from the repository root with:

```sh
node bin/norna.mjs --site-dir examples/feature-demos/media-and-surfaces/site dev:local
```

The site has a home page plus pages for Norna blocks and surfaces. Sitewide
logo handling, banners and footers are demonstrated separately in
`examples/feature-demos/sitewide-content/`.
