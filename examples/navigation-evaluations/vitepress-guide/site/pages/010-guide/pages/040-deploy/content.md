...
## Setting a Public Base Path {#setting-a-public-base-path}
...
- Keep `cleanUrls` off (the default): portable output needs links that end in `.html`, since there is no server to rewrite pretty URLs.
...
- `head` entries are emitted verbatim, as always — avoid root-absolute paths like `/favicon.ico` there and prefer absolute URLs or `transformHead`.
...
- Links created by `createContentLoader` content stay site-absolute (their HTML is embedded into other pages, so no single relative prefix is correct) — they resolve only for a root mount.
